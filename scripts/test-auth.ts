...
async function main() {
  ...
  const TEST_USERS = [
    ...
    { email: 'coordinator@test.com', password: 'password123', expectedRole: 'coordinator' },
  ];
  ...
  if (connError) {
    console.log(`  DB connection test: ${connError.message} (expected - RLS blocks anon)`);
  } else {
  ...
  for (const user of TEST_USERS) {
    ...
    if (profileError) {
      ...
      if (profile.role === user.expectedRole) {
        console.log(`  [OK] Role: ${profile.role} (expected: ${user.expectedRole})`);
      } else {
        console.log(`  [FAIL] Role mismatch: ${profile.role} (expected: ${user.expectedRole})`);
      }
    }
